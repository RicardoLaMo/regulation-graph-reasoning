...
hallucinated citations, paraphrase, negation flip). \textbf{We frame the work
as a diagnostic harness, not a production routing system.} Concretely, the
expected-calibration error drops from $0.124$ to $0.051$ and Mondrian APS
empirical coverage is $0.991$ (overcovered against the $0.90$ target);
applying the documented two-pronged abstention rule (\(|S|\!\geq\!3\) or
\(\hat p_{(1)}\!<\!\theta\)) the system abstains on $69.7\%$ of complaints
and is $89.6\%$ accurate on the routed remainder. \emph{Citation faithfulness
is only $6.1\%$ on clean inputs and $16.9\%$ on the adversarial subset}, a
strong negative result that we expose explicitly rather than smooth over.
We release all code, scripts, and figures under MIT.
\end{abstract}
